...
function App() {
  ...
  useEffect(() => {
    ...
    const updateHeight = () => {
      setMapCardHeight(mapCard.getBoundingClientRect().height)
    }
      ...
      <section className="mx-auto flex min-h-screen max-w-[1500px] items-center px-6 py-16 sm:px-10 lg:px-12">
        <div className="w-full space-y-8">
          ...
          <div className="grid gap-10 lg:grid-cols-[minmax(0,1.45fr)_minmax(360px,0.75fr)] lg:items-stretch">
            ...
            <form
              className={`h-full rounded-[2rem] border p-6 backdrop-blur-xl transition-all duration-300 sm:p-8 ${moodTone.panelClassName}`}
              onSubmit={handleSubmit}
              style={
                mapCardHeight === null
                  ? undefined
                  : { minHeight: `${mapCardHeight}px` }
              }
            >
              <div className="flex h-full flex-col gap-6">
                <div className="flex items-center justify-between gap-4">
                  ...
                  <span
                    className={`rounded-full px-4 py-2 text-sm font-semibold ${moodTone.accentClassName}`}
                  >
                    {moodTone.badge}
                  </span>
                </div>
